...
export function initAuth() {
    ...
    // Prevent default form submission and trigger logic manually to avoid reload
    ...
    loginSubmit.onclick = async (e) => {
        ...
        try {
            ...
            // Redirect to dashboard immediately after login/signup if on a public page
            if (window.location.pathname.includes('index.html') || window.location.pathname.includes('pricing.html') || window.location.pathname.includes('legal-info.html') || window.location.pathname === '/' || window.location.pathname === '') {
                window.location.href = 'dashboard.html' + window.location.search;
            }
        } catch (error) {
            console.error(error);
